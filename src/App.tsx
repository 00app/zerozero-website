...
export default function App() {
  ...
  // Helper function to generate results for non-profile journeys
  const generateJourneyResults = (journey, answers) => {
    // This mirrors the logic from ResultsPage but returns the data
    const baseResults = {
      travel: [
        {
          title: 'cheapest',
  ...
  const handleIntroComplete = () => {
    // Show location prompt after intro, but only if not asked before
  ...
  const handleLocationAllow = (latitude, longitude) => {
  ...
  const handleLocationDeny = () => {
    ...
    // Fallback: could use IP geolocation or manual country selection
    console.log('Location permission denied, using fallback options');
  };
