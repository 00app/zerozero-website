...
const journeyQuestions = {
  ...
  shop: [
    { text: 'clothes frequency?', type: 'pills', key: 'category', options: ['clothes', 'tech', 'home', 'other'] },
...
export function JourneyPage({ journey, onComplete, onBack, onHome }: JourneyPageProps) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      // Reset current values when question changes
      setCurrentValue('');
  ...
  return (
    <div className="zz-journey">
      ...
      <div className="zz-qbox px-4 lg:px-8">
        ...
        >
          <h3 className="text-center lg:text-left" style={{ fontSize: 'var(--h3)', fontWeight: 800, lineHeight: 1.2 }}>
            {displayText}
          </h3>
        </div>
      </div>
